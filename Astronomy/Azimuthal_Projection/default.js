...
function createInstancedStars() {
    ...
    starData.forEach((entry, index) => {
        ...
        let myTest = calcXYZ(entry.RA, entry.DEC);
        positionMatrixT = new THREE.Matrix4();
        positionMatrixT.setPosition(myTest.x, myTest.y, myTest.z);

		size = mapValue(entry.MAG, -2, 6.6, 1.0,0.2);
		positionMatrixT.scale(new THREE.Vector3(size, size, size));
        instancedStars.setMatrixAt(index, positionMatrixT);
...
//function project_sterographic(ra, dec) {
//    let x = Math.cos(dec * Math.PI / 180) * Math.sin(ra * Math.PI / 180);
//    let z = Math.cos(dec * Math.PI / 180) * Math.cos(ra * Math.PI / 180);
//    let y = Math.sin(dec * Math.PI / 180);
//    return new THREE.Vector3(x, y, z);
//}

function projectPoint(point, sphereRadius, planeY = 100){
    let projPoint = new THREE.Vector3();
    projPoint.x = (planeY + 100)*(point.x/(point.y+sphereRadius));
    projPoint.z = (planeY + 100)*(point.z/(point.y+sphereRadius));
    projPoint.y = planeY;
    return projPoint
}
    //    let x = Math.cos(dec * Math.PI / 180) * Math.sin(ra * Math.PI / 180);
...
function createInstancedStars2() {
    ...
    starData.forEach((entry, index) => {
        ...
        positionVector = projectPoint(position, 100);
...
starCultureNames.STELLA.map(key => {
    for(var i = 0; i < starCulture[key].length; i++) {
        ...
        let position = new THREE.Vector3().setFromMatrixPosition(myMat);    
        let newline = createLine(projectPoint(position, 100), new THREE.Vector3(0, -100, 0));
        //scene.add(newline);
...
async function loadSphereData() {
    ...
    starCultureLoaded = starCultureNames;
	createStarLines();
    createStarLines2();
}
...
let points2 = pointsRADec.map(point => {
    ...
    return projectPoint(new THREE.Vector3(x, y, z), 100);
...
for(let i = 0; i < points.length; i++) {
    let newline = createLine(projectPoint(points[i], 100), new THREE.Vector3(0, -100, 0));
    //scene.add(newline);
}
...
loadSphereData().then(() => {
    // Data is loaded and ready for rendering.
    render();
});
...
function render() {
    //console.log(starCultureLoaded);
    if (starCultureLoaded != null){
        //console.log('Star culture loaded');
    }
